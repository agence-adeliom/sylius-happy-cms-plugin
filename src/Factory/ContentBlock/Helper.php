...
class Helper
{
    ...
    private readonly PreviewOverlayRenderer $previewOverlayRenderer;
    ...
    public function __construct(
        ...
        $this->previewOverlayRenderer = new PreviewOverlayRenderer();
    }
    ...
    public function renderContentBlock(ContentBlockInterface $contentBlock, bool $preview = false, array $extra = []): ?Markup
    {
        ...
        // In preview mode, wrap the content with the page builder attributes and overlays
        if ($preview) {
            return new Markup($this->previewOverlayRenderer->wrap(
                $renderedContent,
                $blockId,
                $contentBlock->getLayer(),
                $isPreviewPublished,
                $contentBlock->isDeleted(),
            ), 'UTF-8');
        }
